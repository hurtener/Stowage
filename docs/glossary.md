...
- **`_meta` identity intake** — the ae2 mechanism by which the MCP handlers read the non-authorizing identity dimensions (`user`, `session`, `agent_id`, and — since ae2b, `project`) from the host-injected inbound `_meta` (dockyard v1.8 `server.RequestMeta`). Originally additive (alongside the `project_id`/`user_id`/`session_id` arguments; no `_meta` ⇒ behaviour identical to arg-only); ae2b (D-140/M1) later removed `project_id`/`user_id` from the 13 read/mutate MCP contracts that were the sole D-125 sub-tenant-targeting mechanism, so `_meta`/JWT is now the ONLY channel for project/user narrowing on those 13 tools — `session_id` remains an arg everywhere, and `memory_browse`'s `project_id`/`user_id` args remain (out of ae2b's scope, see `docs/plans/phase-ae2b-contract-removal.md`'s As-built deviations). Centralized in the single helper `mcpserver.readMetaIdentity` (no per-handler copy-paste). Tenant is never read from `_meta` (D-138).
