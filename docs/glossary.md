...
- **Agent→topic policy binding** — a `(tenant_id, agent_id) → {allow_topics, deny_topics}` row set, stored as `(subject_kind='agent', view_name='default')` rows in the non-scope, **general** `topic_views` table (`store.AgentPolicy`, behind the `TopicViewStore` seam / `Store.TopicViews()`, D-146 amended by the checkpoint decision D-151), resolved at read time and fed to ae6's `filterByTopicOwnScope` to curate (not isolate — D-139) an agent's own-scope retrieval. Carries no memory rows and no `user_id`; excluded from the DSAR cascade; generalized to named views (other `subject_kind`/`view_name` values on the same table) by ae9. **Fails open** on a policy-store error (D-139/D-036).
...
- **`_meta` identity intake** — the ae2 mechanism by which the MCP handlers read the non-authorizing identity dimensions (`user`, `session`, `agent_id`, and — since ae2b, `project`) from the host-injected inbound `_meta` (dockyard v1.8 `server.RequestMeta`). Originally additive (alongside the `project_id`/`user_id`/`session_id` arguments; no `_meta` ⇒ behaviour identical to arg-only); ae2b (D-140/M1) later removed `project_id`/`user_id` from the **14** read/mutate MCP contracts that were the sole D-125 sub-tenant-targeting mechanism (the 13 the plan enumerated **plus `memory_browse`/`BrowseInput`**, folded in per the plan's As-built deviations), so `_meta`/JWT is now the ONLY channel for project/user narrowing on those 14 tools — `session_id` remains an arg everywhere. The 4 exclusions (`IngestRecord`/`IngestTargetScope` write-time, `GrantsInput` admin, `ProactiveConfigInput`) keep their fields. Centralized in the single helper `mcpserver.readMetaIdentity` (no per-handler copy-paste). Tenant is never read from `_meta` (D-138).
...
- **Session-REPLACE** — the D-137 session semantics ae2 implements on the MCP surface: the effective session for a call is the host-injected `_meta.session` when set, else the `session_id` argument (`_meta` wins — the `metaElseArg` precedence), fed to the handler's **existing** session sink (`retrieval.Request.SessionID`, `playbook.Options.SessionID`, etc.) — never added as a `Scope.Session` store predicate (which would change results for existing callers). The HTTP counterpart (`X-Harbor-Session`) is deferred to the HTTP/JWT identity work (ae7).
...
- **Handshake methods (MCP)** — the fixed, identity-free JSON-RPC methods (`initialize`, `notifications/initialized`, `ping`, `tools/list`) plus the streamable-HTTP SSE GET leg and session DELETE, served **without** authentication on the MCP-over-HTTP surface when `auth.mode=jwt`. Everything else (notably `tools/call`) requires the per-call bearer; classification ambiguity fails closed to protected. (Phase ae11, D-152.)
