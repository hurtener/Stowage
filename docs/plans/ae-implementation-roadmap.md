...
- [ ] **ae13** — topics resolve tenant-wide: `topics.Service` normalizes every caller scope to tenant-only (matching the write shape), so per-user (jwt) and project-scoped runs extract under the tenant's configured topics instead of silently falling back to the profile default pack; store layer untouched; per-subject curation stays ae9 views · D‑154
