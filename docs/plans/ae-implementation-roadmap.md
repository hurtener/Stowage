...
- [ ] **ae12** — `memory_ingest_run`, the Harbor run-completion sink: a 24th MCP tool mirroring `RunCompletionPayload` format_version 1 exactly; identity from the verified per-call bearer (payload quad cross-checked, fail-closed); conversion in the core (`records.FromRunCompletion`); one extraction buffer per run + eager flush; `memory_ingest` untouched; MCP-only tiering sanctioned · D‑153
