...
func (c *embeddedClient) Remember(ctx context.Context, req RememberRequest) (MemoryReceipt, error) {
	...
	if err != nil { return MemoryReceipt{}, err }
	scope.Session = req.SessionID
	if scope.Session == "" { scope.Session = c.scope.Session }
	r, err := reconcile.Remember(ctx, c.stack.Store, scope, reconcile.ExplicitRequest{SourceRecordID: req.SourceRecordID, Quote: req.Quote, Kind: req.Kind, IdempotencyKey: req.IdempotencyKey}, c.stack.Retriever.Cache())
	if err != nil { return MemoryReceipt{}, err }
	return *r, nil
}
func (c *embeddedClient) Correct(ctx context.Context, req CorrectRequest) (MemoryReceipt, error) {
	...
	scope.Session = req.SessionID
	if scope.Session == "" { scope.Session = c.scope.Session }
	r, err := reconcile.Correct(ctx, c.stack.Store, scope, reconcile.ExplicitRequest{SourceRecordID: req.SourceRecordID, Quote: req.Quote, MemoryID: req.MemoryID, ExpectedRevision: req.ExpectedRevision, IdempotencyKey: req.IdempotencyKey}, c.stack.Retriever.Cache())
	if err != nil { return MemoryReceipt{}, err }
	return *r, nil
}
