...
func guardCommandPG(ctx context.Context, tx pgx.Tx, scope identity.Scope, g *store.CommandGuard, now int64) error {
	...
	if errors.Is(err, pgx.ErrNoRows) || (err == nil && (role != "user" || role != g.Source.Role || content != g.Source.Content || occurred != g.Source.OccurredAt)) {
		return store.ErrCommandConflict
	}
	if err != nil {
		return fmt.Errorf("pgstore: guard source: %w", err)
	}
	for _, target := range g.Targets {
		...
		if errors.Is(err, pgx.ErrNoRows) || (err == nil && store.MemoryRevision(*actual) != store.MemoryRevision(target)) {
			return store.ErrCommandConflict
		}
		if err != nil {
			return fmt.Errorf("pgstore: guard target: %w", err)
		}
	}
	return nil
}
