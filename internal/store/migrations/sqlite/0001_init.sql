-- Phase 03 day-one schema — SQLite dialect (D-037, D-038)
-- Timestamps: INTEGER unix-millis. IDs: TEXT (ULID). Booleans: INTEGER 0/1.
-- PRAGMAs (WAL, foreign_keys, busy_timeout) are set by the driver, not here.
...
CREATE TABLE IF NOT EXISTS buffer_items (
    ...
    token_estimate INTEGER NOT NULL DEFAULT 0,
    flushed_at     INTEGER NOT NULL DEFAULT 0,
    created_at     INTEGER NOT NULL
);
