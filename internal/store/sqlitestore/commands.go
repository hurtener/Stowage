...
func (e *eventStore) Get(ctx context.Context, scope identity.Scope, id string) (*store.Event, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlitestore: get event: %w", err)
	}
	return &ev, nil
}
...
func guardCommandSQLite(tx *sql.Tx, scope identity.Scope, g *store.CommandGuard, now int64) error {
	...
	if ss.Project != scope.Project || ss.User != scope.User || g.Source.TenantID != scope.Tenant {
		return store.ErrCommandConflict
	}
	...
	if errors.Is(err, sql.ErrNoRows) || (err == nil && (role != "user" || role != g.Source.Role || content != g.Source.Content || occurred != g.Source.OccurredAt)) {
		return store.ErrCommandConflict
	}
	if err != nil {
		return fmt.Errorf("sqlitestore: guard source: %w", err)
	}
	for _, target := range g.Targets {
		if target.TenantID != scope.Tenant || target.ProjectID != scope.Project || target.UserID != scope.User {
			return store.ErrCommandConflict
		}
		...
		if errors.Is(err, sql.ErrNoRows) || (err == nil && store.MemoryRevision(*actual) != store.MemoryRevision(target)) {
			return store.ErrCommandConflict
		}
		if err != nil {
			return fmt.Errorf("sqlitestore: guard target: %w", err)
		}
	}
	return nil
}
