...
func TestAgentCatalogGoldens(t *testing.T) {
	...
	if len(list.Tools) != len(want) { t.Fatalf("agent catalog has %d tools", len(list.Tools)) }
	for _, tl := range list.Tools {
		if !want[tl.Name] { t.Fatalf("unexpected planner tool: %s",tl.Name) }; delete(want,tl.Name)
		data, err := json.MarshalIndent(tl,"","  "); if err != nil {t.Fatal(err)}
		path := filepath.Join("testdata","agent",tl.Name+".json")
		if os.Getenv("UPDATE_GOLDEN") == "1" { if err := os.MkdirAll(filepath.Dir(path),0755);err!=nil{t.Fatal(err)};if err := os.WriteFile(path,append(data,'\n'),0644);err!=nil{t.Fatal(err)} } else { checkGolden(t,path,data) }
		var in map[string]any; raw,_:=json.Marshal(tl.InputSchema);if err:=json.Unmarshal(raw,&in);err!=nil{t.Fatal(err)}
		props,_:=in["properties"].(map[string]any)
		for key,value:=range props { p,_:=value.(map[string]any); if p["description"]==nil {t.Errorf("%s.%s lacks guidance",tl.Name,key)} }
		if tl.Name=="memory_retrieve" && len(props)!=2 {t.Errorf("runtime fields leaked into recall: %v",props)}
	}
}
...
func TestAgentCatalogRejectsRuntimeAndAdminCalls(t *testing.T) {
	...
	for _,name := range []string{"memory_ingest","memory_ingest_run","memory_assert","memory_grants","memory_views","memory_flush","memory_forget"} {
		...
		if err==nil && !r.IsError {t.Errorf("hidden operation %s was callable",name)}
	}
}

func TestAgentSchemaRejectsInvalidValuesBeforeHandlers(t *testing.T) {
	session:=agentSession(t,newTestServices(t))
	for _,tc:=range []struct{name string;args map[string]any}{
		...
		r,err:=session.CallTool(context.Background(),&mcpsdk.CallToolParams{Name:tc.name,Arguments:tc.args});if err==nil&&!r.IsError{t.Errorf("accepted invalid %s: %v",tc.name,tc.args)}
	}
}
...
func TestAgentMissingSourceDoesNotClaimSave(t *testing.T) {
	...
	if err!=nil {if !strings.Contains(err.Error(),"source_required"){t.Fatal(err)};return}
	if !r.IsError {t.Fatal("missing source was accepted")}
	b,_:=json.Marshal(r.Content); if !strings.Contains(string(b),"source_required"){t.Fatalf("unhelpful error: %s",b)}
}
