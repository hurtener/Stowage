...
func NewAgent(info server.Info, svc *Services) (*server.Server, error) {
	...
	if err := declare[AgentRetrieveInput, RetrieveOutput]("memory_retrieve").Describe(toolDescription("memory_retrieve")).Handler(func(ctx context.Context, in AgentRetrieveInput) (tool.Result[RetrieveOutput], error) {
		limit := in.Limit; if limit == 0 { limit = 6 }
		return makeRetrieveHandler(svc)(ctx, RetrieveInput{Query: in.Query, Limit: limit})
	}).Register(srv); err != nil { return nil, err }
	if err := declare[InspectInput, InspectOutput]("memory_inspect").Describe(toolDescription("memory_inspect")).Handler(makeInspectHandler(svc)).Register(srv); err != nil { return nil, err }
	if err := declare[RememberInput, reconcile.ExplicitReceipt]("memory_remember").Describe(toolDescription("memory_remember")).Handler(func(ctx context.Context, in RememberInput) (tool.Result[reconcile.ExplicitReceipt], error) {
		...
		if err != nil { return tool.Result[reconcile.ExplicitReceipt]{}, err }
	...
	if err := declare[PlaybookInput, PlaybookOutput]("memory_playbook").Describe(toolDescription("memory_playbook")).Handler(makePlaybookHandler(svc)).Register(srv); err != nil { return nil, err }
	return srv, nil
}
...
func makeInspectHandler(svc *Services) tool.Handler[InspectInput, InspectOutput] {
	return func(ctx context.Context, in InspectInput) (tool.Result[InspectOutput], error) {
		if (in.MemoryID == "") == (in.Citation == "") { return tool.Result[InspectOutput]{}, fmt.Errorf("inspect requires exactly one returned memory_id or citation") }
		drill, err := makeDrilldownHandler(svc)(ctx, DrilldownInput{MemoryID: in.MemoryID, Citation: in.Citation})
		if err != nil { return tool.Result[InspectOutput]{}, err }
		...
		if out.Sources == nil { out.Sources = []DrilldownSpan{} }
		if len(out.Sources) == 0 { out.Warning = "No source spans are available. Do not treat this item as verified user evidence." }
...
func boundExplicit(ctx context.Context, operation string, req reconcile.ExplicitRequest) (reconcile.ExplicitRequest, error) {
	...
	if raw, present := meta["stowage"]; present {
		...
		if !ok { return req, fmt.Errorf("invalid host source binding") }
		for _, key := range []string{"source_record_id", "idempotency_key", "operation"} {
			if v, present := binding[key]; present { if _, ok := v.(string); !ok { return req, fmt.Errorf("invalid host source binding") } }
		}
		if op, _ := binding["operation"].(string); op != "" && op != operation { return req, fmt.Errorf("host source binding is for another operation") }
		if id, _ := binding["source_record_id"].(string); id != "" {
			if req.SourceRecordID != "" && req.SourceRecordID != id { return req, fmt.Errorf("source argument conflicts with host binding") }
			req.SourceRecordID = id
		}
		req.IdempotencyKey, _ = binding["idempotency_key"].(string)
	}
	if req.SourceRecordID == "" { return req, fmt.Errorf("source_required: the host must persist the actual user message and bind its record ID, or provide a user-source record ID returned by memory_inspect; no memory was saved") }
	return req, nil
}
...
func readerJSON(value any) string {
	...
	if err != nil { return "Memory content unavailable: could not render the result. Do not infer missing evidence." }
