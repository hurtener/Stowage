...
func (d *declaration[I, O]) Register(srv *server.Server) error {
	if d.handler == nil { return fmt.Errorf("%s: missing handler", d.name) }
...
func DescribeInputJSON(name string, raw []byte) ([]byte, error) {
	...
	if err := json.Unmarshal(raw, &schema); err != nil { return nil, err }
	decorateProperties(schema)
	props, _ := schema["properties"].(map[string]any)
	set := func(key, field string, value any) { if p, ok := props[key].(map[string]any); ok { p[field] = value } }
	...
	if p, ok := props["kinds"].(map[string]any); ok { if items, ok := p["items"].(map[string]any); ok { items["enum"] = kinds } }
	...
	case "memory_retrieve":
		...
		set("limit", "minimum", 0); set("limit", "maximum", 100)
	...
	case "memory_inspect":
		...
		set("memory_id", "minLength", 1); set("citation", "minLength", 1)
	case "memory_assert": enum("action", "add", "update", "delete"); enum("kind", append([]string{""}, kinds...)...)
	case "memory_topics": enum("action", "", "list", "upsert", "delete")
	case "memory_review": enum("action", "list", "approve", "reject")
	case "memory_resolve": enum("action", "confirm", "reject")
	case "memory_branch": enum("action", "fork", "merge", "discard")
	case "memory_browse": enum("mode", "", "recent", "superseded")
	case "memory_causal": enum("direction", "", "backward", "forward", "both")
	case "memory_feedback": enum("signal", "use", "save", "fail", "noise", "wrong_citation")
	case "memory_flush": enum("trigger", "", "explicit", "session_end")
	case "memory_suggestions": enum("action", "", "list", "accept", "dismiss")
	case "memory_proactive_config": enum("action", "", "get", "set")
	case "memory_agent_policy": enum("action", "create", "get", "list", "delete")
	case "memory_views": enum("action", "create_view", "update_view", "delete_view", "list_views")
	case "memory_grants":
		enum("action", "create_group", "list_groups", "add_member", "remove_member", "list_members", "create_grant", "list_grants", "revoke_grant")
		enum("access", "", "read", "contribute"); enum("zone_ceiling", "", "public", "work")
	}
...
func decorateProperties(schema map[string]any) {
	...
	for key, raw := range props {
		p, ok := raw.(map[string]any); if !ok { continue }
		if _, exists := p["description"]; !exists { if desc := parameterDescriptions[key]; desc != "" { p["description"] = desc } }
		decorateProperties(p)
		if items, ok := p["items"].(map[string]any); ok { decorateProperties(items) }
	}
}
