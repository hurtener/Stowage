...
func explicitWrite(ctx context.Context, st store.Store, scope identity.Scope, operation string, req ExplicitRequest, inv []ScopeInvalidator, attempt int) (*ExplicitReceipt, error) {
	if st == nil || scope.Tenant == "" { return nil, store.ErrScopeRequired }
	if err := validateExplicit(operation, req); err != nil { return nil, err }
	...
	if errors.Is(err, store.ErrNotFound) { return nil, ErrInvalidEvidence }
	if err != nil { return nil, fmt.Errorf("remember: read evidence: %w", err) }
	...
	if operation == "correct" {
		...
		if target.ProjectID != owner.Project || target.UserID != owner.User { return nil, store.ErrNotFound }
		if target.Status != "active" || store.MemoryRevision(*target) != req.ExpectedRevision {
			...
			if r, e := readExplicitReceipt(ctx, st, owner, receiptID, digest); e == nil { return r, nil } else if !errors.Is(e, store.ErrNotFound) { return nil, e }
			return nil, store.ErrCommandConflict
		}
		if source.OccurredAt <= 0 || source.OccurredAt < target.ValidFrom { return nil, ErrInvalidEvidence }
		junctions, err := st.Memories().GetJunctions(ctx, owner, target.ID)
		if err != nil { return nil, fmt.Errorf("correct: snapshot evidence: %w", err) }
		...
		if NormalizeContent(target.Content) == NormalizeContent(req.Quote) {
			...
			if mem.PrivacyZone == "" { mem.PrivacyZone = "personal" }
		...
		if findErr != nil && !errors.Is(findErr, store.ErrNotFound) { return nil, findErr }
		if findErr == nil && existing.ProjectID == owner.Project && existing.UserID == owner.User && existing.SessionID == source.SessionID {
			...
			if err != nil { return nil, err }
			if len(j.Provenance) == 0 { return nil, fmt.Errorf("%w: matching memory lacks provenance; inspect it before reconciliation", store.ErrCommandConflict) }
			mem = *existing
	...
	if err != nil { return nil, fmt.Errorf("remember: encode receipt: %w", err) }
	...
	if err := st.Memories().Commit(ctx, owner, cs); err != nil {
		if errors.Is(err, store.ErrCommandReplay) { return readExplicitReceipt(ctx, st, owner, receiptID, digest) }
...
func validateExplicit(operation string, req ExplicitRequest) error {
	...
	if operation == "correct" {
		if req.MemoryID == "" || len(req.MemoryID) > 256 || len(req.ExpectedRevision) != 64 || req.Kind != "" { return fmt.Errorf("%w: correct requires memory_id and expected_revision from inspection; kind is inherited", ErrInvalidCommand) }
		if _, err := hex.DecodeString(req.ExpectedRevision); err != nil { return ErrInvalidCommand }
	} else if req.MemoryID != "" || req.ExpectedRevision != "" { return fmt.Errorf("%w: use correct to replace an existing memory", ErrInvalidCommand) }
	switch req.Kind {
	case "", "fact", "preference", "decision", "gotcha", "pattern", "task", "narrative", "strategy", "failure_mode":
	default: return fmt.Errorf("%w: unsupported kind", ErrInvalidCommand)
	}
	return nil
}
...
func explicitDigest(operation string, req ExplicitRequest) string {
	req.IdempotencyKey = ""
	if operation == "remember" && req.Kind == "" { req.Kind = "fact" }
	b, _ := json.Marshal(struct { Operation string; Request ExplicitRequest }{operation, req})
	h := sha256.Sum256(b)
...
func readExplicitReceipt(ctx context.Context, st store.Store, scope identity.Scope, id, digest string) (*ExplicitReceipt, error) {
	...
	if ev.Type != "memory.command_committed" || json.Unmarshal([]byte(ev.Payload), &saved) != nil || saved.Version != 1 || saved.Receipt.ReceiptID != id { return nil, fmt.Errorf("remember: invalid stored command receipt") }
	if saved.RequestHash != digest { return nil, ErrIdempotencyConflict }
...
func observeExplicit(ctx context.Context, st store.Store, scope identity.Scope, receipt *ExplicitReceipt) {
	...
	if err != nil {
		...
		if errors.Is(err, store.ErrNotFound) { receipt.CurrentStatus, receipt.StatusDegraded = "unavailable", false }
		return
	}
