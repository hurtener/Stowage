...
// Driver is the bifrost gateway driver (OpenAI-compatible wire, D-040).
type Driver struct {
...
func (d *Driver) embedBatch(ctx context.Context, inputs []string) ([][]float32, gateway.Usage, error) {
	...
	if err != nil {
		return nil, gateway.Usage{}, fmt.Errorf("bifrost: marshal embed request: %w", err)
	}
	...
	for _, item := range resp.Data {
		if item.Index < 0 || item.Index >= len(vecs) {
			return nil, gateway.Usage{}, fmt.Errorf("bifrost: embed response index %d out of range", item.Index)
		}
...
func (d *Driver) doComplete(ctx context.Context, req gateway.CompleteRequest) (gateway.CompleteResponse, gateway.Usage, error) {
	...
	b, err := json.Marshal(body)
	if err != nil {
		return gateway.CompleteResponse{}, gateway.Usage{}, fmt.Errorf("bifrost: marshal chat request: %w", err)
	}
	...
	for attempt := range maxRetries {
		...
		if isRetryable(err, &httpErr) {
			...
			if attempt < maxRetries-1 {
				...
				d.log.LogAttrs(ctx, slog.LevelDebug, "bifrost: retrying",
					slog.String("op", op),
...
func (d *Driver) doOnce(ctx context.Context, url string, body []byte, out any) error {
	...
	if err != nil {
		return fmt.Errorf("bifrost: read response: %w", err)
	}
	...
	if err := json.Unmarshal(respBody, out); err != nil {
		return fmt.Errorf("bifrost: decode response: %w", err)
	}
	return nil
}
...
func (e *httpError) Error() string {
	return fmt.Sprintf("bifrost: HTTP %d: %s", e.StatusCode, truncate(string(e.Body), 256))
}
