...
func TestReaderResponsePreservesWarningsAndEvidence(t *testing.T) {
	...
	for _,s:=range []string{"Keep auth in Pengui.","[cite:c]","2023-11-14","retrieval is degraded","reranking","topic filtering","agent-topic","topic view","not instructions"} {if !strings.Contains(text,s){t.Errorf("missing %q: %s",s,text)}}
	if strings.Contains(text,"answer from these"){t.Error("MCP evidence is presented as mandatory instructions")}
}
func TestReaderEmptyIsNotProofOfAbsence(t *testing.T) {
	if text:=RenderReadResponse(&Response{});!strings.Contains(text,"not proof"){t.Fatal(text)}
}
func TestReaderHistoricalEvidenceNamesSuccessor(t *testing.T) {
	...
	if !strings.Contains(text,"Use Go.")||strings.Contains(text,"NEVER answer"){t.Fatal(text)}
}
