...
# An optional identity.Scope.Agent (read-path only) + a (tenant_id, agent_id)→topic
# policy binding feed ae6's own-scope fail-open filter to narrow a tenant's OWN
# retrieval by the calling agent — with ZERO scope-table schema change and ZERO
# write-path change. Agent read-filter on {SDK,HTTP,MCP}; policy admin on {HTTP,MCP}.
#
# Verifies:
...
#   AC-4  AgentPolicyStore seam + 0013 migration on both drivers.
...
#   AC-10 retrieval.agent_filter_enabled is a registered knob (default false).
...
if grep -Rql 'ADD COLUMN.*agent\|agent_id .*NOT NULL' internal/store/migrations 2>/dev/null \
   | grep -qv '0013_agent_topic_policies'; then
  failc "AC-2: an agent column was added to a scope-table migration"
else
  ok "AC-2: no agent column added to any scope table"
fi

# ── AC-4: policy store seam + migration on both drivers ─────────────────────────
if grep -q 'AgentPolicyStore' internal/store/store.go; then
  ok "AC-4: AgentPolicyStore seam declared"
else
  skip "AC-4: AgentPolicyStore not declared yet"
fi
mig=0
for d in sqlite postgres; do
  [ -f "internal/store/migrations/$d/0013_agent_topic_policies.sql" ] || { mig=1; skip "AC-4: $d 0013 migration missing"; }
done
[ "$mig" -eq 0 ] && ok "AC-4: 0013_agent_topic_policies migration present for both drivers"
...
# ── AC-10: knob registered ──────────────────────────────────────────────────────
if grep -q 'agent_filter_enabled' internal/config/config.go; then
  ok "AC-10: retrieval.agent_filter_enabled present in config"
else
  skip "AC-10: retrieval.agent_filter_enabled not present yet"
fi
